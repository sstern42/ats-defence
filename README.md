...
Page analytics is a separate thing sat next to all of that, and it is a script tag in `index.html` rather than anything the game knows about. Every event above starts at `session_started`, which is the game booting, so nothing in the store can see somebody who opened the page and left, or count the arrivals a post produced. That is the top row every funnel in `docs/` is missing and the whole of what the script is for. It adds nothing to the fifteen, reads nothing from them, and the two are never joined.

Links out of the game carry campaign parameters, which is the same five keys the game already reads on the way in and puts on `session_started`. `src/config/links.js` holds the campaign and the list of hosts allowed to carry it, and `src/services/links.js` is the only file that puts one on a URL. The list is the rule rather than a convenience, and it has one host on it. The music credit is off it because it points at somebody else's page and their report is not ours to write in. The tip jar is off it because Ko-fi reports no campaign back, so the parameters would be decoration on a link somebody is about to follow, and what the game wanted to know about that click is already on `kofi_clicked`, which carries the screen and the wave it came from.
