...
Links out of the game carry campaign parameters, which is the same five keys the game already reads on the way in and puts on `session_started`. `src/config/links.js` holds the campaign and the list of hosts allowed to carry it, and `src/services/links.js` is the only file that puts one on a URL. The list is the rule rather than a convenience: the tip jar and the site this is a project of are tagged, the music credit is not, because it points at somebody else's page and their report is not ours to write in. `utm_content` is the screen the link was on, spelled exactly as `from_screen` is on `kofi_clicked`, so the two records of one click line up.
