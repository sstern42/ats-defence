...
One link out of the game carries campaign parameters, which are the same keys the game already reads on the way in and puts on `session_started`. It is the footer link back to the site this is a project of, and the tags are in the constant in `src/config/links.js`. The other two are bare on purpose: the music credit points at somebody else's page and their report is not ours to write in, and the tip jar reports no campaign back, so a tag there would be decoration on a link somebody is about to follow when what the game wanted to know about that click is already on `kofi_clicked`.
